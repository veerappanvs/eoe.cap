													...
													<td>
														<table class="table white-background no-border left-aligned">
															<c:forEach items="${strength.elements}" step="3"
																...
																<tr>
																	<td ><form:checkbox label="${strength.elements.get(elementStatus.index).label }"
																			path="strengths[${strengthStatus.index}].elements[${elementStatus.index}].selected"
																			 />
																		</td>
																	<td ><form:checkbox label="${strength.elements.get(elementStatus.index+1).label }"
																			path="strengths[${strengthStatus.index}].elements[${(elementStatus.index+1)}].selected"
																			 />
																		</td>
																	<td ><form:checkbox label="${strength.elements.get(elementStatus.index+2).label }"
									...
									<div id="growthTable">
										...
										<table class="table white-background">
													...
													<td>
														<table class="table white-background no-border left-aligned">
															<c:forEach items="${growth.elements}" step="3"
																var="element" varStatus="elementStatus">
																<tr>
													 				<td ><form:checkbox label="${growth.elements.get(elementStatus.index).label }"
																			path="growths[${growthStatus.index}].elements[${elementStatus.index}].selected"
																			 />
																		</td>
																	<td ><form:checkbox label="${growth.elements.get(elementStatus.index+1).label }"
																			path="growths[${growthStatus.index}].elements[${(elementStatus.index+1)}].selected"
																			 />
																		</td>
																	<td ><form:checkbox label="${growth.elements.get(elementStatus.index+2).label }"
											...
											<tr>
												<td colspan="4"><input type="button" value="Add Row"
													name="Add" class="btn btn-primary"
													onclick="addRow('growthTable','2')" /></td>
											</tr>
											</c:if>
										</table>
									</div>
