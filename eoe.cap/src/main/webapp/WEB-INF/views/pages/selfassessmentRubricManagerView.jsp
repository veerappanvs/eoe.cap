		...
		<div class="cap-tabs">
			<ul class="nav nav-tabs" id="myTab" role="tablist">
				<c:forEach items="${assessmentInfo.elements}"
					...
					<li class="nav-item"><a
						class="nav-link ${elementIndex.index eq 1?'active':'' }"
						id="${element.elementCode}-tab" data-toggle="tab"
						href="#${element.elementCode}-info" role="tab"
						aria-controls="${element.elementCode}-info" aria-selected="true"><div
								style="width: 120px; height:75px">${element.altDesc}</div></a></li>
				</c:forEach>
			...
			<div class="tab-content" id="myTabContent">
				<c:forEach items="${assessmentInfo.elements}"
					var="element" varStatus="elementIndex">
					<div
						class="tab-pane fade ${elementIndex.index eq 1?'show active':'' }"
							...
							<tbody>
								<c:forEach items="${element.dimensions}" var="dimension"
									...
									<tr>
										...
										<c:forEach items="${element.ratingInfos}" var="ratingInfo">
											<td style="text-align: center"><form:radiobutton disabled="true"
